...
ok.0:
	...
	%tmp.13 = load %Main* (%Main*,i32) *, %Main* (%Main*,i32) ** %tmp.12
	%tmp.14 = call %Main*(%Main*, i32 ) %tmp.13( %Main* %tmp.8, i32 %tmp.7 )
...
entry:
	...
	%tmp.24 = call i8*(i32 ) @malloc( i32 %tmp.23 )
...
okay:
	...
	store %_Main_vtable* @_Main_vtable_prototype, %_Main_vtable** %tmp.26
	store %Main* %tmp.25, %Main** %tmp.21
	%tmp.27 = getelementptr %Main, %Main* %tmp.25, i32 0, i32 1
	store i32 0, i32* %tmp.27
